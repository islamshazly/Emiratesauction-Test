...
final class NetworkManager: NSObject {
  ...
  class func startRequest<T: BaseModel>(withRequestConfiguration requestConfiguration : RequestConfiguraton , withMappingObject objectClass : T? , successClouser success : NetworkSuccessClosure? , andFailure failure : @escaping NetworkFailureClosure ) {
    ...
    if !serverReachable(forUrl: baseURL) {
    ...
    Alamofire.request(baseURL, method: requestConfiguration.requestMethod, parameters: requestConfiguration.parameters, encoding: encode, headers: requestConfiguration.headers).responseJSON { (response :DataResponse<Any>) in
...
//            print("Request Paramters: \(String(describing: model.toJSON()))")
      ...
      case .failure(_):
        failure((response.result.error as? TError) ?? (TError.TNetworkFaild(reason: TError.NetworkFaildReasons.ServerError)))
        break
      }
    }
  }
